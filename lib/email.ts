...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f8fafc;color:#1e293b;margin:0;padding:0">
  <div style="max-width:560px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
    <div style="background:#08111f;padding:24px 32px;text-align:center">
      <span style="color:#f97316;font-size:22px;font-weight:800;letter-spacing:-0.5px">My Bonnet</span>
    ...
    <div style="background:#f8fafc;padding:16px 32px;text-align:center;font-size:12px;color:#94a3b8;border-top:1px solid #e2e8f0">
      &copy; ${new Date().getFullYear()} My Bonnet &middot; South Africa
    </div>
  </div>
</body></html>`;
}
...
export async function sendMagicLinkEmail(to: string, url: string) {
  await send(to, "Your sign-in link for My Bonnet", layout(`
    <h2 style="font-size:22px;font-weight:800;color:#0f172a;margin:0 0 8px">Sign in to My Bonnet</h2>
...
export async function sendWelcomeEmail(to: string, name: string, isWorkshop: boolean) {
  const subject = isWorkshop ? "Your workshop is live on My Bonnet" : "Welcome to My Bonnet";
  const content = isWorkshop
    ...
    : `<h2 style="font-size:22px;font-weight:800;color:#0f172a;margin:0 0 8px">Welcome to My Bonnet, ${name}! 🚗</h2>
